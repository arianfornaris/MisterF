...
- Mister F is an English-learning product. The revised quiz must practice and evaluate English as the target skill.
...
Only use the kinds listed above. There is no supported quiz_true_false, short-answer, or essay item yet. If the teacher asks for variety or all available block types, cover the supported kinds above.
