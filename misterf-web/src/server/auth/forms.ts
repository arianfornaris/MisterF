...
export async function renderHome(request: Request, response: Response): Promise<void> {
  ...
  const requestedChatRoomShareId =
    typeof requestedChatRoomShareIdRaw === 'string'
  ...
  const requestedChatRoomId =
    typeof requestedChatRoomIdRaw === 'string'
      ? requestedChatRoomIdRaw.trim()
      : '';
  const requestedChatRoomConversationIdRaw = request.params.roomConversationId;
  const requestedChatRoomConversationId =
    typeof requestedChatRoomConversationIdRaw === 'string'
  ...
  let currentView: 'chat' | 'chatrooms' | 'practiceModules' | 'profiles' =
  ...
  let chatRoomPageMode: 'list' | 'new' | 'edit' | 'history' | 'conversation' | 'share' =
    'list';
  let selectedChatRoom: StoredChatRoom | null = null;
  let selectedSharedChatRoom: StoredChatRoom | null = null;
  let selectedChatRoomShareLink: StoredChatRoomShareLink | null = null;
  let selectedChatRoomSharedFromProfileName = '';
  let selectedChatRoomConversation: StoredChatRoomConversation | null = null;
  let selectedChatRoomCharacters: ReturnType<typeof listChatRoomCharacters> = [];
  let selectedChatRoomMessages: ReturnType<typeof listChatRoomMessages> = [];
  let chatRoomConversations: ReturnType<typeof listChatRoomConversationsForRoom> = [];
  ...
  if (requestedChatRoomConversationId && user) {
    const conversation = findChatRoomConversationForUser(
      requestedChatRoomConversationId,
      user.id,
    );
    if (!conversation) {
      response.redirect('/chatrooms');
  ...
  const shareTargetChatRoomProfiles =
    availableProfiles.filter(
  ...
  const chatRoomShareUrl = activeChatRoomShareLink
    ? buildAbsoluteAppUrl(
        `/chatrooms/shared/${encodeURIComponent(activeChatRoomShareLink.id)}`,
      )
    : '';
...
export function handleUpdateChatRoom(
  request: Request,
  response: Response,
): void {
  const user = request.authUser;
...
export function handleJoinChatRoom(
  request: Request,
  response: Response,
): void {
  const user = request.authUser;
...
export function handleCreateChatRoomConversation(
  request: Request,
  response: Response,
): void {
  const user = request.authUser;
...
export function handleShareChatRoomToProfile(
  request: Request,
  response: Response,
): void {
  const user = request.authUser;
...
export function handleAcceptSharedChatRoomLink(
  request: Request,
  response: Response,
): void {
...
function readChatRoomFormPayload(request: Request): {
  title: string;
  description: string;
  characters: Array<{
    name: string;
  ...
  const characters = [1, 2, 3]
    .map((index) => ({
      ...
      shortDescription: readField(
        request.body[`characterShortDescription${index}`],
      ),
    }))
