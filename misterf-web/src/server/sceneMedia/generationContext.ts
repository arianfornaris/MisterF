...
export function createSceneMediaGenerationSourceContext(input: {
  ...
  return {
    audio: input.sourceItem.audio ? {
      ...
      speakers: Array.from(new Set(
        input.sourceItem.audio.clips.map((clip) => clip.speaker),
      )),
...
export function buildSceneMediaSourceContextPrompt(
  ...
  const imageRule = {
    ...
    generate_new:
      'Replace the image according to the active user request while keeping it compatible with every kept layer.',
    keep_existing:
      'Treat the existing image as an immutable visual anchor; generated text must agree with what is visible.',
  }[context.layerDecisions.image];
  const scriptRule = {
    do_not_include: 'Do not assume, recreate, or depend on a script or audio layer.',
    generate_new:
      'Replace the script and audio content according to the active user request; use the old script only for continuity that the request does not supersede.',
    keep_existing:
      'Treat the existing script and audio facts, roles, identities, and spoken names as immutable compatibility anchors.',
