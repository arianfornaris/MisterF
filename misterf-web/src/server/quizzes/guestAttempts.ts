 ...
 */
function findGuestAttemptSharePath(quizId: string): string | null {
 ...
 */
export function buildGuestAttemptExitPath(quizId: string): string {
  return findGuestAttemptSharePath(quizId) ?? '/';
}

export type PendingGuestQuizEvaluation = {
  quizTitle: string;
 ...
 */
export function findPendingGuestQuizEvaluation(
  returnTo: string,
): PendingGuestQuizEvaluation | null {
  let url: URL;
  try {
