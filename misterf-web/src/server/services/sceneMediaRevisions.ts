...
export type SceneMediaRevisionPlan = z.infer<typeof sceneMediaRevisionSchema>;

export async function generateSceneMediaRevisionPlan(input: {
  conversationHistory: SceneMediaAuthoringMessage[];
  currentMedia: SceneMediaLibraryItem;
  imageBytes: Buffer;
  imageContentType: string;
  instructionLanguage?: Locale;
  openRouterApiKey: string;
  prompt: string;
}): Promise<SceneMediaRevisionPlan> {
...
function normalizePlan(
  plan: SceneMediaRevisionPlan,
  media: SceneMediaLibraryItem,
): SceneMediaRevisionPlan {
  ...
  return {
    ...
    scriptAndAudioDecision:
      plan.scriptAndAudioDecision === 'keep_existing' && !hasScriptAndAudio
        ? 'do_not_include'
