...
export function buildTutorPracticeModuleTools(input: {
  ...
  return {
    list_practice_modules: tool({
      description:
        'List the saved practice modules in the current profile only when the learner explicitly asks to find, list, show, open, or choose saved modules. Do not use this tool for normal tutoring, practice inside a module, or progress through the current lesson flow. Optionally filter by a text query in the title or description.',
    ...
    delete_practice_module: tool({
      description:
        'Delete a saved practice module from the current profile only when the learner explicitly asks to delete or remove that saved module. Do not use this tool during normal tutoring or because a practice session is finished. Existing chats keep their historical snapshot.',
      inputSchema: z.object({
        practiceModuleId: z.string().trim().min(1)
