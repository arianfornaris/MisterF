...
export function buildTutorConversationTools(input: {
  ...
  const titleLanguageRule = conversationTitleLanguageRule(
    ...
    update_conversation_title: tool({
      ...
      execute: async ({ reason, title }) => {
        ...
        const renamedConversation = renameConversationForUser(
          ...
          userId,
          normalizedTitle,
          { updatedByUser: isExplicitUserRequest },
        );
