...
export function recordQuizAttemptProgress(attempt: StoredQuizAttempt): void {
  ...
  const locale =
    findProfileForUser(attempt.profileId, attempt.userId)?.instructionLanguage ??
    defaultLocale;
  const event = upsertLearnerProgressEvent({
    ...
    summary: compactText(
      translate(locale, 'progress.quizEventSummary', {
...
export function refreshLearnerProgressSummary(input: {
  ...
  const locale =
    findProfileForUser(input.profileId, input.userId)?.instructionLanguage ??
    defaultLocale;
...
export function buildLearnerProgressSummary(
  ...
  const recommendedPractice = uniqueLimited(
    events.flatMap((event) => [
      ...event.details.practiced.map((item) =>
        translate(locale, 'progress.keepPracticing', { item }),
      ),
      ...event.details.recommendations,
    ]),
    10,
