...
export type StoredResourceParticipationSummary = {
  generatedAt: string;
  inputFingerprint: string;
  resourceId: string;
  summaryText: string;
 ...
 */
export function getResourceParticipationSummary(
  resourceId: string,
): StoredResourceParticipationSummary | null {
  const row = getDb()
    .prepare(
      `
...
function addResourceSourceNoticeMessage(
  ...
  const fallbackTitle = translate(
    input.locale,
    input.type === 'quiz' ? 'msg.resourceQuizLabel' : 'msg.resourceRoleplayLabel',
  );
