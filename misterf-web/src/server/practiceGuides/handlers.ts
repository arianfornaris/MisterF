 ...
 */
export async function handleGeneratePracticeGuideParticipationSummary(
  request: Request,
  response: Response,
): Promise<void> {
  const resolved = resolveOwnPracticeGuide(request, response);
    ...
    const result = await generateGuideParticipationSummary({
      ...
      request: {
        ...
        reports: reports.map((report) =>
          summarizeGuideReportForParticipation(report.report),
        ),
 ...
 */
function summarizeGuideReportForParticipation(
  report: Record<string, unknown> | null,
): { difficultyAreas: string[]; nextSteps: string[]; practicedTopics: string[] } {
...
function renderPracticeGuideNewView(
  ...
  input: {
    ...
    originFolder?: OriginFolderContext;
    user: NonNullable<Request['authUser']>;
  },
): void {
...
export async function handlePreviewPracticeGuideModification(
  ...
  try {
    ...
    setPendingModification<PracticeGuideDraft, PracticeGuideDraft>(owner, {
...
export function handleApplyPracticeGuideModification(
  ...
  const pending = getPendingModification<PracticeGuideDraft, PracticeGuideDraft>(owner);
