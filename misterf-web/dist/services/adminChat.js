...
import { createActivity, findActivityForUser, findAdminChatThreadForUser, findConversationForUser, listActivitiesForProfile, listConversationsForActivity, listConversationsForProfile, renameAdminChatThreadForUser, renameConversationForUser, deleteActivityForUser, updateActivity, } from '../db/repository.js';
...
let adminCapabilitiesInstruction;
...
function getAdminCapabilitiesInstruction() {
    adminCapabilitiesInstruction ??= fs.readFileSync(path.join(env.projectRoot, 'gameplays/admin-chat-capabilities.md'), 'utf8');
...
export async function runAdminChatLoop(history, options) {
    ...
    for (let turn = 0; turn < maxAdminTurns; turn += 1) {
        ...
        try {
            const result = await generateText({
                ...
                tools: buildAdminChatTools(options.userId, options.profileId, options.currentThreadId ?? null),
            });
            ...
            const blocks = mergeAdminLinkBlocks(validateAdminResponseBlocks(parsedObject), inferredLinks);
...
function buildAdminChatTools(userId, profileId, currentThreadId) {
    return {
        list_activities: tool({
            description: 'List the user activities. Optionally filter by a text query in the title or description.',
            ...
            execute: async ({ query }) => {
                ...
                const activities = listActivitiesForProfile(userId, profileId)
                    ...
                    .map((activity) => ({
                    conversationCount: listConversationsForActivity(activity.id, userId, profileId).length,
        ...
        update_activity: tool({
            description: 'Update an existing activity. Use list_activities first if you need to discover the activity id.',
            inputSchema: z.object({
                activityId: z.string().trim().min(1),
            ...
            execute: async ({ activityId, description, title, tutorInstructions }) => {
                ...
                return {
                    activity: summarizeActivity(next, listConversationsForActivity(next.id, userId, profileId).length),
                    url: `/activities/${encodeURIComponent(next.id)}`,
                };
            },
        }),
        delete_activity: tool({
            description: 'Delete an existing activity. Existing chats keep their historical snapshot, but the activity is removed from the activity library.',
            inputSchema: z.object({
                activityId: z.string().trim().min(1),
            }),
        ...
        list_conversations: tool({
            description: 'List the user conversations. Optionally filter by a query in the title.',
            inputSchema: z.object({
        ...
        build_activity_link: tool({
            description: 'Build a UI activity link action so the interface can render an activity button instead of showing a raw URL.',
            inputSchema: z.object({
                activityId: z.string().trim().min(1),
        ...
        auto_title_current_chat: tool({
            description: 'Assign a better title to the current admin chat thread when its purpose is already clear.',
            inputSchema: z.object({
                title: z.string().trim().min(1).max(160),
            }),
            execute: async ({ title }) => {
                ...
                const renamedThread = renameAdminChatThreadForUser(currentThreadId, userId, title);
                if (!renamedThread) {
                    return {
...
function summarizeActivity(activity, conversationCount) {
    return {
        conversationCount,
        description: activity.description,
        id: activity.id,
...
function validateAdminResponseBlocks(value) {
...
function mergeAdminLinkBlocks(blocks, inferredLinks) {
    const merged = [...blocks];
    const existingKeys = new Set(merged
        .filter((block) => block.type === 'activity_link')
...
function extractInferredLinkBlocks(toolResults) {
    const actions = [];
    for (const result of toolResults) {
        ...
        if (result.toolName === 'build_activity_link') {
            const action = extractActionRecord(result.output?.action);
            if (action) {
                actions.push({
        ...
        if (result.toolName === 'create_activity' || result.toolName === 'update_activity') {
            const activity = extractActivityRecord(result.output?.activity);
            if (activity) {
                actions.push({
...
function extractActionRecord(value) {
...
function extractActivityRecord(value) {
    if (!value || typeof value !== 'object') {
        return null;
    }
    const record = value;
