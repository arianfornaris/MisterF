...
import { buildLlmRequestTokenUsage, logLlmInvalidRawResponse, logLlmRequest, logLlmResponse, logLlmToolCalls, } from '../llmTutor/logging.js';
...
export async function runSecretaryAgentLoop(history, options) {
    ...
    }, effectiveResult.finishReason, effectiveResult.usage, effectiveResult.providerMetadata, 1);
...
function buildSecretaryLessonTools(input) {
    ...
    return {
        ...
        return_to_tutor: tool({
            description: 'Return control of the conversation to Mr. F when the learner asks to go back to the tutor.',
            inputSchema: z.object({
...
function extractReturnToTutor(toolResults) {
    return toolResults.some((result) => {
        ...
        return action.type === 'return_to_tutor';
    });
}
