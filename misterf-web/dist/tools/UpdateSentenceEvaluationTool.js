import { completeSentenceChallenge, createSentenceChallenge, findActiveSentenceChallenge, listMessages, listSentenceChallenges, updateMessageMetadata, upsertSentenceAttempt, } from '../db/repository.js';
const fallbackCompletionScore = 1;
export class UpdateSentenceEvaluationTool {
    name = 'update_sentence_evaluation';
    declaration = {
        name: this.name,
        description: 'ALTA PRIORIDAD, USO MANDATORIO: guarda una evaluación visual por partes del último intento del usuario. Esta es la herramienta que controla todo el ciclo de vida del reto: si no hay reto abierto, la app abre uno automáticamente y guarda esta evaluación como el primer intento; si todas las partes tienen status "correct", la app marca automáticamente la oración como completada, actualiza Intentos y lanza confeti. OBLIGATORIA en cada interacción donde el usuario presente un intento de traducción o una corrección, sin excepción, antes de dar retroalimentación textual. Úsala aunque la oración esté correcta, incorrecta o casi perfecta. NO USAR para preguntas laterales, selección de tema/nivel, saludos, explicaciones generales ni mensajes que no sean intentos de traducción. Si omites esta tool, la app pierde el registro visual y pedagógico del intento. Las partes deben reconstruir la oración del usuario en el mismo orden, sin duplicar, omitir ni inventar texto. Esta evaluación visual no reemplaza tu respuesta normal en el chat.',
        parametersJsonSchema: {
            type: 'object',
            properties: {
                parts: {
                    type: 'array',
                    description: 'Segmentos naturales que reconstruyen la oración del usuario en el mismo orden. No necesariamente palabra por palabra. No corrijas el texto dentro de los segmentos; conserva lo que escribió el usuario.',
                    items: {
                        type: 'object',
                        properties: {
                            text: {
                                type: 'string',
                                description: 'Fragmento exacto de la oración del usuario. Puede ser una palabra, expresión o grupo natural.',
                            },
                            status: {
                                type: 'string',
                                enum: ['correct', 'improve', 'error'],
                                description: 'correct si la parte está correcta; improve si se entiende pero puede sonar más natural, precisa o idiomática; error si está incorrecta o si contiene una palabra mal escrita, por ejemplo "cal" en vez de "call".',
                            },
                            explanation: {
                                type: 'string',
                                description: 'Explicación breve en español. Obligatoria para improve y error; omítela o déjala vacía para correct.',
                            },
                        },
    ...
    execute(call, context) {
        ...
        const message = updateMessageMetadata(context.lastUserMessageId, context.conversationId, { sentenceEvaluation: evaluation });
        if (!message) {
        ...
        if (activeChallenge) {
            const isCorrect = evaluation.parts.every((part) => part.status === 'correct');
            ...
            if (isCorrect) {
                const completedChallenge = completeSentenceChallenge(activeChallenge.id, context.conversationId, fallbackCompletionScore);
...
function normalizeSentenceEvaluation(args) {
...
function createFallbackChallenge(conversationId, lastUserMessageId) {
    const sourceSentence = inferLatestSourceSentence(conversationId, lastUserMessageId) ??
...
function inferLatestSourceSentence(conversationId, lastUserMessageId) {
...
function extractBestSpanishQuotedSentence(content) {
...
function isLikelySpanishChallengeSentence(value) {
    ...
    const englishSignals = /\b(the|would|like|ticket|please|where|nearest|bus|stop|train|to|from|round-trip|return)\b/i;
    return spanishSignals.some((pattern) => pattern.test(value)) &&
        !englishSignals.test(value);
}
function normalizeSentencePart(value) {
    ...
    return {
        explanation: explanation || 'Esta parte necesita ajuste para sonar correcta o natural.',
        status,
        text,
    };
}
...
//# sourceMappingURL=UpdateSentenceEvaluationTool.js.map