...
export function getLearnerCharacter(draft) {
    return draft.characters.find((character) => character.id === 'learner')
        ?? draft.characters[0];
}
export function getAiCharacter(draft) {
...
export function buildRoleplayProgressSummary(result) {
    return compactText(result.summary, 280);
}
export function buildRoleplayAuthoringMessage(role, content, draftSnapshot) {
    const message = {
...
export function appendRoleplayAuthoringMessages(existingMessages, ...messages) {
...
async function generateStructuredRoleplayOutput(input) {
    ...
    for (let turn = 0; turn < maxRoleplayGenerationTurns; turn += 1) {
        ...
        if (!parsed.success) {
            ...
            if (turn < maxRoleplayGenerationTurns - 1) {
                appendRoleplayCorrectionRequest(messages, 'Your previous JSON did not match the required schema.', result.text);
                continue;
            }
...
function appendRoleplayCorrectionRequest(messages, reason, invalidOutput) {
    messages.push({
...
//# sourceMappingURL=roleplays.js.map