...
function appendChatRoomConversationReportCorrectionRequest(messages, input) {
    const invalidOutput = input.invalidOutput?.trim();
    if (invalidOutput) {
        messages.push({
...
function appendChatRoomPracticeModuleCorrectionRequest(messages, input) {
    const invalidOutput = input.invalidOutput?.trim();
    if (invalidOutput) {
        messages.push({
            content: invalidOutput.slice(0, 10000),
...
function buildCharacterRoster(characters) {
    return characters
        .map((character) => [
        `- ${character.name}`,
...
function resolveCharacterByName(characters, speakerName) {
    const exactMatch = characters.find((character) => character.name === speakerName);
    if (exactMatch) {
        return exactMatch;
    }
    const normalizedTarget = speakerName.trim().toLowerCase();
    return (characters.find((character) => character.name.trim().toLowerCase() === normalizedTarget) || null);
}
        ...
        for (let turn = 0; turn < maxChatRoomGenerationTurns; turn += 1) {
            ...
            if (!parsed.success) {
                ...
                if (turn < maxChatRoomGenerationTurns - 1) {
                    appendChatRoomStructuredCorrectionRequest(messages, {
                        invalidOutput: result.text,
                        reason: 'Your previous JSON did not match the required schema for the chatroom response.',
                        turn: turn + 1,
                    });
                }
                continue;
            ...
            if (turn < maxChatRoomGenerationTurns - 1) {
                appendChatRoomStructuredCorrectionRequest(messages, {
                    invalidOutput: result.text,
                    reason: 'The JSON was valid, but after validation there were no usable messages. Re-emit a complete valid block with real speaker names and non-empty content.',
                    turn: turn + 1,
                });
            }
        }
