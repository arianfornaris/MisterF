...
export async function ensureOpenRouterKeyForUser(userId) {
...
export async function getOpenRouterApiKeyForUser(userId) {
...
export function getOpenRouterKeyRecordForUser(userId) {
    const row = findUserOpenRouterKeyRow(userId);
    return row ? toOpenRouterUserKeyRecord(row) : null;
}
export async function getOpenRouterRemoteKeyInfoForUser(userId) {
    const row = findUserOpenRouterKeyRow(userId);
    if (!row?.key_hash) {
        return null;
    }
...
export async function updateOpenRouterUserKeyLimit(input) {
    ...
    if (!row?.key_hash) {
        throw new OpenRouterKeyProvisioningError('Este usuario todavía no tiene una key de OpenRouter administrable.');
    }
    ...
    getDb()
        ...
        WHERE user_id = ?
      `)
        .run(remoteInfo.limit ?? input.limitUsd, remoteInfo.limitReset ?? input.limitReset, remoteInfo.disabled ? 'disabled' : 'active', input.userId);
    return remoteInfo;
}
async function provisionOpenRouterKeyForUser(userId) {
    if (!env.openrouterManagementApiKey) {
        markOpenRouterKeyError(userId, 'Missing OPENROUTER_MANAGEMENT_API_KEY for user key provisioning.');
        throw new OpenRouterKeyProvisioningError('Falta configurar OPENROUTER_MANAGEMENT_API_KEY para crear keys por usuario.');
    }
    if (!env.openrouterKeyEncryptionSecret) {
        markOpenRouterKeyError(userId, 'Missing OPENROUTER_KEY_ENCRYPTION_SECRET for user key storage.');
        throw new OpenRouterKeyProvisioningError('Falta configurar OPENROUTER_KEY_ENCRYPTION_SECRET para guardar keys de OpenRouter.');
    }
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : 'Unknown OpenRouter key error.';
...
async function createOpenRouterApiKey(name) {
    ...
    const keyHash = typeof json.data?.hash === 'string'
        ? json.data.hash
...
function findUserOpenRouterKeyRow(userId) {
    const row = getDb()
        .prepare(`
        SELECT
          user_id,
...
async function getOpenRouterRemoteKeyInfo(keyHash) {
    const response = await fetch(`${env.openrouterBaseUrl}/keys/${encodeURIComponent(keyHash)}`, {
        headers: {
            authorization: `Bearer ${env.openrouterManagementApiKey}`,
        },
        method: 'GET',
    ...
    if (!response.ok || !json.data) {
        throw new OpenRouterKeyProvisioningError(parseOpenRouterMessage(json, response.status));
    }
    return toOpenRouterRemoteKeyInfo(json.data);
}
async function patchOpenRouterRemoteKey(keyHash, body) {
    const response = await fetch(`${env.openrouterBaseUrl}/keys/${encodeURIComponent(keyHash)}`, {
        body: JSON.stringify(body),
        headers: {
            authorization: `Bearer ${env.openrouterManagementApiKey}`,
...
function toOpenRouterUserKeyRecord(row) {
    return {
...
function parseOpenRouterMessage(json, status) {
...
function readLimitResetValue(value) {
    if (value === null) {
        return null;
    }
...
function markOpenRouterKeyPending(userId, name) {
    getDb()
        .prepare(`
        ...
        ON CONFLICT(user_id) DO UPDATE SET
          ...
          updated_at = CURRENT_TIMESTAMP
      `)
        .run(userId, name, env.openrouterUserKeyLimitUsd, null);
}
function saveOpenRouterUserKey(input) {
    getDb()
        ...
        ON CONFLICT(user_id) DO UPDATE SET
          ...
          updated_at = CURRENT_TIMESTAMP
      `)
        .run(input.userId, input.keyHash, encryptApiKey(input.apiKey), input.name, env.openrouterUserKeyLimitUsd, null);
}
function markOpenRouterKeyError(userId, message) {
    getDb()
        .prepare(`
...
//# sourceMappingURL=openRouterUserKeys.js.map