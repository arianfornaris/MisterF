...
export const quizMetadataSchema = z
    ...
    .strict();
export function safeParseQuizMetadata(value) {
...
export function quizDraftToMetadata(draft) {
 ...
 */
export function applyQuizMetadataToDraft(previousDraft, metadata) {
 ...
 */
export function applyQuizBlocksAndSectionsToDraft(previousDraft, blocks, sections) {
 ...
 */
export function diffQuizBlocks(before, after) {
...
export function quizBlocksDiffHasChanges(diff) {
...
export function findQuizBlock(draft, blockId) {
 ...
 */
export function setQuizBlockItem(draft, blockId, item) {
    if (!draft.blocks.some((block) => block.id === blockId)) {
        return draft;
    }
    return quizDraftSchema.parse({
        ...draft,
        blocks: draft.blocks.map((block) => block.id === blockId ? { ...block, item } : block),
    });
}
 ...
 */
export function insertQuizBlock(draft, item, placement = {}) {
    ...
    const newBlock = sectionId
        ? { id: blockId, item, sectionId }
        : { id: blockId, item };
    const blocks = placement.position === 'start'
        ? [newBlock, ...draft.blocks]
        : [...draft.blocks, newBlock];
    return {
        blockId,
        draft: canonicalizeQuizDraftBlockOrder(quizDraftSchema.parse({ ...draft, blocks })),
    };
}
...
export function buildQuizResponsesSummary(input) {
    ...
    const questions = input.draft.blocks.map((block) => {
 ...
 * prompt.
 */
export function computeQuizResponsesFingerprint(attempts) {
    let evaluatedCount = 0;
    let latestUpdatedAt = '';
