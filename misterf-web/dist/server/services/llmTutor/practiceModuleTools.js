...
export function buildTutorPracticeModuleTools(input) {
    ...
    return {
        list_practice_modules: tool({
            description: 'List the practice modules in the current profile. Optionally filter by a text query in the title or description.',
        ...
        update_practice_module: tool({
            description: 'Update a practice module in the current profile. If practiceModuleId is omitted, use the current practice module when this conversation already belongs to one. Any provided title, description, or tutorInstructions must be written in Spanish. Never write those fields in English.',
            inputSchema: z.object({
        ...
        delete_practice_module: tool({
            description: 'Delete a practice module from the current profile. Existing chats keep their historical snapshot.',
            inputSchema: z.object({
                practiceModuleId: z.string().trim().min(1).optional(),
            }),
        ...
        build_practice_module_link: tool({
            description: 'Build a practice module link block for the UI so the chat can render a button to open a practice module. Only call this with a real existing practiceModuleId obtained from tool results or the current chat context. Never invent, guess, infer, slugify, or paraphrase a practiceModuleId.',
            inputSchema: z.object({
                practiceModuleId: z.string().trim().min(1).optional(),
