import { tool } from 'ai';
import { z } from 'zod';
import { createPracticeGuide, deletePracticeGuideForUser, findPracticeGuideForUser, listConversationsForPracticeGuide, listPracticeGuidesForProfile, updatePracticeGuide, } from '../../db/repository.js';
        ...
        list_practice_guides: tool({
            ...
            execute: async ({ query }) => {
                ...
                const practiceGuides = listPracticeGuidesForProfile(userId, profileId)
                    ...
                    return normalizeSearchText(`${practiceGuide.title}\n${practiceGuide.description}\n${practiceGuide.tutorInstructions}`).includes(normalizedQuery);
                })
...
function summarizePracticeGuide(practiceGuide, userId) {
    return {
...
export function extractInferredPracticeGuideLinkBlocks(toolResults) {
    const actions = [];
    for (const result of toolResults) {
        ...
        if (result.toolName === 'build_practice_guide_link') {
            const action = extractPracticeGuideLinkAction(result.output?.action);
            if (action) {
                actions.push(action);
            }
            continue;
        ...
        if (result.toolName === 'create_practice_guide' || result.toolName === 'update_practice_guide') {
            const practiceGuide = extractPracticeGuideRecord(result.output?.practiceGuide);
            if (practiceGuide) {
                actions.push({
...
function extractPracticeGuideLinkAction(value) {
...
function extractPracticeGuideRecord(value) {
    if (!value || typeof value !== 'object') {
        return null;
    }
    const record = value;
...
function buildPracticeGuideLinkLabel(title) {
    return `Abrir guía de práctica: ${title}`;
}
//# sourceMappingURL=practiceGuideTools.js.map