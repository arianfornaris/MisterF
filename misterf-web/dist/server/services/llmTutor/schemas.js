...
const forbiddenTutorDialogueSpeakerTokenPattern = /(?:^|\s)(ai|assistant|asistente|maestro|profesor|profesora|teacher|tutor)(?:\s|$)/;
function isForbiddenTutorDialogueSpeakerName(value) {
    const normalized = value
        ...
        .replace(/[^a-z0-9]+/g, ' ')
        .trim();
    return (forbiddenTutorDialogueSpeakerNames.has(normalized) ||
...
export const dialogueCharacterMessageBlockSchema = z
    ...
    message: 'dialogue_character_message.name must be an invented fictional character, not Mr. F, the tutor, teacher, assistant, or AI.',
    path: ['name'],
});
export const dialogueTranscriptBlockSchema = z
    ...
    for (const [index, turn] of block.turns.entries()) {
        ...
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'dialogue_transcript.turns speaker must be a fictional character or learner label, not Mr. F, the tutor, teacher, assistant, or AI.',
...
const tutorPlanStepIdSchema = z
    ...
    .regex(/^[a-z][a-z0-9_-]*$/, 'step id must start with a lowercase letter and contain only lowercase letters, numbers, underscores, or hyphens.');
...
export const translateToEnglishPromptBlockSchema = z
    ...
    .strict();
function countSentencePlaceholders(sentence, placeholder) {
    if (!placeholder) {
        return 0;
    }
...
export const sentenceEvaluationBlockSchema = z
    ...
    if (block.correction && !block.parts.some((part) => part.status !== 'correct')) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: 'sentence_evaluation.correction may only be present when at least one part is marked improve or error.',
            path: ['correction'],
        });
    }
})
