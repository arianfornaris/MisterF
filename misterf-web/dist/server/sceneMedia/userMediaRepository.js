...
export function saveUserSceneMediaGeneratedLayers(input) {
    getDb()
        .prepare(`
        ...
        WHERE id = ?
      `)
        .run(input.image ? JSON.stringify(input.image) : null, input.mediaId);
...
export function retryUserSceneMediaGenerationJob(input) {
    ...
    `);
    const transaction = db.transaction(() => {
        insertJob.run(jobId, mediaRow.id, mediaRow.user_id, mediaRow.profile_id, previousJob?.type ?? 'new_media', mediaRow.generation_prompt, mediaRow.generation_mode, mediaRow.script_type_preference, mediaRow.format, mediaRow.level, mediaRow.source_media_id, previousJob?.layer_decisions_json ?? null);
