...
const emptyPracticeGuideFormValues = {
    description: '',
    title: '',
    tutorInstructions: '',
};
function getDefaultPracticeGuideFormValues(pageKind, selectedPracticeGuide) {
...
async function buildPracticeGuidesPageModel(request, response, pageKind) {
    ...
    let selectedSharedPracticeGuide = null;
    ...
    if (pageKind === 'share') {
        ...
        selectedSharedPracticeGuide = findPracticeGuideById(legacyShareLink.practiceGuideId);
...
async function renderPracticeGuidesPage(request, response, pageKind, overrides = {}) {
    ...
    response.render('practice-guides', {
        ...
        practiceGuideFormValues: getDefaultPracticeGuideFormValues(pageKind, viewModel.selectedPracticeGuide),
...
export async function handleGeneratePracticeGuideDraft(request, response) {
    ...
    try {
        ...
        await renderPracticeGuidesPage(request, response, 'new', {
            ...
            practiceGuideGenerationError: isCreditError
...
export function renderSharedPracticeGuidePage(request, response) {
...
export function handleCreatePracticeGuide(request, response) {
    const user = request.authUser;
    const activeProfile = request.activeProfile;
    if (!user?.emailVerified || !activeProfile) {
        response.redirect('/login');
...
export async function handleRevisePracticeGuide(request, response) {
    ...
    try {
        ...
        response.redirect(`/practice-guides/${encodeURIComponent(updatedPracticeGuide.id)}/edit?aiRevision=success`);
    }
    catch (error) {
        const isCreditError = isCreditExhaustedError(error);
        await renderPracticeGuidesPage(request, response.status(422), 'edit', {
...
export function handleAcceptSharedPracticeGuideLink(request, response) {
    const shareId = String(request.params.shareId || '').trim();
    if (!shareId) {
        response.redirect('/resources');
        return;
