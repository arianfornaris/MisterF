...
import { buildAppShellContext, buildDocumentTitle, getHomeAuthMessage, } from '../pages/shell.js';
 ...
 */
export function renderHomePage(request, response) {
...
function renderLearningHomePage(request, response) {
    const user = request.authUser;
    const activeProfile = request.activeProfile;
...
function renderTeachingHomePage(request, response) {
    const user = request.authUser;
    const activeProfile = request.activeProfile;
    if (!user || !activeProfile) {
        response.redirect('/login');
 ...
 */
export function handleSwitchHomeMode(request, response) {
    const user = request.authUser;
