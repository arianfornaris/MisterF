...
export function startGoogleLogin(request, response) {
...
export async function finishGoogleLogin(request, response) {
    if (!isGoogleConfigured()) {
        renderGoogleConfigurationError(response);
        return;
    }
...
async function signInGoogleUser(request, response, user, returnTo) {
...
async function getGoogleUserInfo(code) {
    ...
    if (!tokenResponse.ok || typeof tokenJson.access_token !== 'string') {
        throw new Error(typeof tokenJson.error_description === 'string'
...
function verifyGoogleState(state, storedState) {
