...
import { addResourceToFolder, archiveResourceForUser, createResourceFolder, findResourceAccessForProfile, findResourceById, findResourceForUser, findResourceFolderForResource, findResourceShareLinkById, findProfileById, findProfileForUser, findQuizById, findRoleplayById, getOrCreateResourceShareLink, grantResourceAccess, listAccessibleResourceFolderPath, listResourceFolderItems, listResourceFoldersForProfile, listResourcesForProfile, listSharedResourcesForProfile, removeResourceFromFolder, restoreResourceForUser, setResourceShareLinkCollectResults, updateResourceFolder, } from '../db/repository.js';
...
import { createResourceCopyLink, findActiveCopyOfResourceForProfile, findResourceCopyLinkById, revokeResourceCopyLink, } from '../db/repository.js';
...
function buildSharedHowItWorksKeys(type, isSignedIn) {
    if (type === 'quiz') {
        return [
 ...
 */
export function buildSharedResourcePageLocals(request, resource, input) {
    const user = request.authUser;
    const activeProfile = request.activeProfile;
    const typeLabelKeys = {
    ...
    const summary = resource.description.trim()
    ...
    const roleplayCharacters = resource.type === 'roleplay'
    ...
    return {
        ...
        folderItems,
        howItWorksKeys: input.howItWorksKeys ?? buildSharedHowItWorksKeys(resource.type, isSignedIn),
        isExample,
        quizBlockCount,
        roleplayCharacters,
        sharedByName,
 ...
 */
export function handleCreateResourceCopyLink(request, response) {
    const auth = ensureVerifiedResourceUser(request, response);
    if (!auth) {
        return;
    }
...
export function handleRevokeResourceCopyLink(request, response) {
    const auth = ensureVerifiedResourceUser(request, response);
    if (!auth) {
        return;
    }
 ...
 */
function resolveCopyLink(copyLinkId) {
 ...
 * author's resource.
 */
export function renderResourceCopyPage(request, response) {
    const user = request.authUser;
    const activeProfile = request.activeProfile;
    const isSignedIn = Boolean(user?.emailVerified && activeProfile);
 ...
 */
export function handleAcceptResourceCopyLink(request, response) {
    const resolved = resolveCopyLink(readField(request.params.copyLinkId, 120));
    if (!resolved) {
        response.redirect('/resources');
        return;
