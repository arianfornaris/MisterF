...
import { listRoleplayModificationChanges, } from './modificationChanges.js';
...
function readRoleplayResultActionError(value, locale) {
...
function buildAttemptListItems(attempts, locale) {
...
export function handleApplyRoleplayModification(request, response) {
    ...
    const updatedRoleplay = updateRoleplayWithDraft(resolved.roleplay, resolved.user.id, pending.proposed);
    if (!updatedRoleplay) {
        response.status(422).json({
            error: translate(request.locale, 'msg.saveRoleplayError'),
        });
 ...
 */
export async function handleStartSharedRoleplayAttempt(request, response) {
