...
function loadRoleplayCharacterAvatars() {
    const registryPath = path.join(env.projectRoot, 'src/server/roleplays/characters.json');
    const parsed = roleplayCharacterAvatarRegistrySchema.parse(JSON.parse(fs.readFileSync(registryPath, 'utf8')));
    const ids = new Set();
...
const roleplayCharacterAvatarIds = new Set(roleplayCharacterAvatars.map((avatar) => avatar.id));
export function listRoleplayCharacterAvatars() {
    return roleplayCharacterAvatars;
}
export function findRoleplayCharacterAvatar(id) {
    if (!id) {
        return null;
    }
    return roleplayCharacterAvatars.find((avatar) => avatar.id === id) ?? null;
}
export function isRoleplayCharacterAvatarId(value) {
    return roleplayCharacterAvatarIds.has(value);
}
export function normalizeRoleplayCharacterAvatarId(value) {
...
export function buildRoleplayCharacterAvatarPromptOptions() {
    return roleplayCharacterAvatars
        .map((avatar) => (`- ${avatar.id}: ${avatar.name}, age ${avatar.age}, ${avatar.gender}. ${avatar.shortDescription}`))
        .join('\n');
}
//# sourceMappingURL=avatarRegistry.js.map