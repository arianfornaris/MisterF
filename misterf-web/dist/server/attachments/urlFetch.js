...
import { maxUploadBytes, maxUrlRedirects, urlFetchTimeoutMs, } from './limits.js';
...
function readBodyWithCap(response, capBytes) {
    return new Promise((resolve, reject) => {
        ...
        response.on('error', () => reject(new AttachmentRejectedError('url_fetch_failed')));
    });
}
async function requestOnce(url) {
    ...
    return new Promise((resolve, reject) => {
        const request = transport.request(url, {
            ...
            readBodyWithCap(response, maxUploadBytes.url).then((body) => resolve({ body, kind: 'body' }), reject);
        });
        request.on('timeout', () => {
            request.destroy();
            reject(new AttachmentRejectedError('url_fetch_failed'));
        });
        request.on('error', () => reject(new AttachmentRejectedError('url_fetch_failed')));
        request.end();
    });
}
...
//# sourceMappingURL=urlFetch.js.map