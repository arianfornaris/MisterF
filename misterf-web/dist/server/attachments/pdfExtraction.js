...
export async function extractPdf(bytes) {
    const warnings = [];
    const document = await getDocumentProxy(new Uint8Array(bytes)).catch(() => null);
    if (!document) {
    ...
    const extracted = await extractText(document, { mergePages: true }).catch(() => null);
    const text = normalizeWhitespace(typeof extracted?.text === 'string'
        ? extracted.text
        : (extracted?.text ?? []).join('\n'));
...
//# sourceMappingURL=pdfExtraction.js.map