...
import { AttachmentExtractionFailedError, extractAttachmentText, } from '../services/attachmentExtraction.js';
import { logger } from '../services/logger.js';
import { prepareUploadedAttachment, prepareUrlAttachment, } from './ingestion.js';
import { translateRejection, translateWarnings } from './messages.js';
import { approveStagedAttachment, discardStagedAttachment, stageAttachment, } from './stagingStore.js';
import { AttachmentRejectedError, isAttachmentRejectedError, } from './types.js';
/**
 ...
 */
function allowProcessing(userId) {
...
function requireActor(request, response) {
 ...
 */
function toReviewView(staged, locale) {
    return {
...
function respondToFailure(error, request, response) {
...
function readUserPrompt(request) {
...
export async function handleProcessUploadedAttachment(request, response) {
...
export async function handleProcessUrlAttachment(request, response) {
    const actor = requireActor(request, response);
    if (!actor) {
        return;
    }
    ...
    const userPrompt = typeof request.body?.prompt === 'string' ? request.body.prompt : undefined;
    try {
        const prepared = {
...
function readAttachmentId(request) {
 ...
 */
export function handleApproveAttachment(request, response) {
    const actor = requireActor(request, response);
    if (!actor) {
        return;
    }
    const correctedText = typeof request.body?.text === 'string' ? request.body.text : undefined;
    let staged;
    try {
...
export function handleDiscardAttachment(request, response) {
    const actor = requireActor(request, response);
    if (!actor) {
        return;
    }
