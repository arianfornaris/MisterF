 ...
 */
function safeDisplayName(fileName, fallback) {
    const base = fileName
        .split(/[\\/]/)
        .pop()
        ?.replace(/[\u0000-\u001f\u007f]/g, '')
...
export async function prepareUploadedAttachment(upload) {
    ...
    if (sniffed.sourceType === 'image') {
        const normalized = await normalizeImageAttachment(upload.bytes, sniffed.contentType);
        return {
...
export async function prepareUrlAttachment(rawUrl) {
