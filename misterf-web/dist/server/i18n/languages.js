...
export const languages = {
    es: {
        ...
        tutor: {
            ...
            directionOptionsLettered: '`a) practicar vocabulario`, `b) practicar con frases`, `c) hacer una mini conversación`',
            translationExerciseBlocksInline: ', `translate_to_english_prompt`, `understand_in_spanish_prompt`',
            ...
            conversationTitleRule: 'The title must be short, Spanish, human-friendly, and specific; avoid generic titles such as "Práctica de inglés", "Conversación", or "Resumen de conversación".',
        ...
        leakagePatterns: {
            translation: /\btraduce(?:\s+(?:la\s+)?(?:siguiente\s+)?(?:frase|oraci[oó]n|texto))?\s+al\s+ingl[eé]s\b\s*:?/i,
            unscramble: /\b(?:ordena|reordena)\b[\s\S]{0,180}\b(?:palabras|oraci[oó]n|frase)\b/i,
            orderSentences: /\b(?:ordena|reordena|pon)\b[\s\S]{0,180}\b(?:pasos|oraciones|frases|instrucciones|eventos)\b/i,
            matching: /\b(?:une|relaciona|empareja)\b[\s\S]{0,180}\b(?:con|cada|correct[ao]s?|significado|traducci[oó]n|pareja)\b/i,
            multipleChoice: /\b(?:elige|escoge|selecciona|marca)\b[\s\S]{0,180}\b(?:opci[oó]n correcta|respuesta correcta|la correcta)\b/i,
            openWriting: /\b(?:escrib(?:e|es|a|as|an|ir|ir[ií]a(?:s|n)?|iendo)|redact(?:a|as|an|e|es|en|ar|ar[ií]a(?:s|n)?)|crea(?:r|s|n)?|forma(?:r|s|n)?|constru(?:ye|yes|ya|yas|yan|ir|ir[ií]a(?:s|n)?))\b[\s\S]{0,180}\b(?:oraci[oó]n(?:es)?|frase(?:s)?|respuesta|p[aá]rrafo|texto|ejemplo)\b/i,
            revision: /\b(?:corrige(?:s|n)?|corrija(?:s|n)?|corregir(?:[ií]a(?:s|n)?)?|reescrib(?:e|es|a|as|an|ir|ir[ií]a(?:s|n)?|iendo))\b[\s\S]{0,180}\b(?:oraci[oó]n(?:es)?|frase(?:s)?|respuesta|p[aá]rrafo|texto|ejemplo)\b/i,
            ownWords: /\b(?:respond(?:e|es|a|as|an|er|er[ií]a(?:s|n)?)|contest(?:a|as|an|e|es|en|ar|ar[ií]a(?:s|n)?))\b[\s\S]{0,180}\bcon\s+tus\s+propias\s+palabras\b/i,
            correctionAnalysisPolite: /\b(?:puedes|podr[ií]as|podr[ií]an|puede[sn]?)\s+(?:decirme|decirnos|identificar|se[nñ]alar|explicar|indicar|encontrar)\b[\s\S]{0,280}\b(?:error(?:es)?|equivocaci[oó]n(?:es)?|problema(?:s)?)\b[\s\S]{0,280}\b(?:corregir(?:lo|la|los|las)?|corregir[ií]as|corregir[ií]an|corrige(?:lo|la|los|las)?|corriges|corrigen|correcci[oó]n|correcciones)\b/i,
            correctionAnalysisDirect: /\b(?:cu[aá]l(?:es)?\s+(?:es|son)\s+(?:el|los)?\s*error(?:es)?|encuentra\s+(?:el|los)?\s*error(?:es)?|identifica\s+(?:el|los)?\s*error(?:es)?)\b[\s\S]{0,280}\b(?:corregir(?:lo|la|los|las)?|corregir[ií]as|corregir[ií]an|corrige(?:lo|la|los|las)?|corriges|corrigen|correcci[oó]n|correcciones)\b/i,
            correctionKeywords: /\b(?:corrige|correcci[oó]n|correcciones|errores|reescribe|reescribir|int[eé]ntalo)\b/i,
    ...
    en: {
        ...
        tutor: {
            ...
            directionOptionsLettered: '`a) practice vocabulary`, `b) practice with sentences`, `c) do a mini conversation`',
            ...
            conversationTitleRule: 'The title must be short, English, human-friendly, and specific; avoid generic titles such as "English practice", "Conversation", or "Conversation summary".',
            quizEvaluationSupportLanguageRules: '',
            quizAuthoringSupportLanguageRules: '',
        },
        greetings: {
        ...
        leakagePatterns: {
            translation: /\btranslate\b(?:\s+(?:the\s+)?(?:following\s+)?(?:sentence|phrase|text|this|that|it))?\s+(?:in)?to\s+english\b\s*:?/i,
            unscramble: /\b(?:unscramble|reorder|rearrange|arrange)\b[\s\S]{0,180}\b(?:words?|sentence)\b/i,
            orderSentences: /\b(?:order|reorder|arrange|put)\b[\s\S]{0,180}\b(?:steps?|sentences?|events?|instructions?)\b[\s\S]{0,60}\bin\s+(?:the\s+)?(?:right\s+|correct\s+)?order\b/i,
            matching: /\b(?:match|pair|connect|link)\b[\s\S]{0,180}\b(?:with|each|correct|meaning|translation|pair)\b/i,
            multipleChoice: /\b(?:choose|pick|select|mark)\b[\s\S]{0,180}\b(?:correct\s+(?:option|answer)|right\s+(?:option|answer)|the\s+correct\s+one)\b/i,
            openWriting: /\b(?:write|compose|create)\b[\s\S]{0,180}\b(?:sentences?|phrases?|answer|paragraphs?|text|examples?)\b/i,
            revision: /\b(?:rewrite|fix)\b[\s\S]{0,180}\b(?:sentences?|phrases?|answer|paragraphs?|text|examples?)\b|\bcorrect\s+(?:this|the|your|these|that|it|them)\b[\s\S]{0,160}\b(?:sentences?|phrases?|answer|paragraphs?|text|examples?)\b/i,
            ownWords: /\b(?:answer|respond|explain|describe)\b[\s\S]{0,180}\bin\s+your\s+own\s+words\b/i,
            correctionAnalysisPolite: /\b(?:can|could)\s+you\s+(?:tell\s+me|point\s+out|identify|find|spot|explain)\b[\s\S]{0,280}\b(?:error|mistake|problem)s?\b[\s\S]{0,280}\b(?:correct|fix|rewrite)\b/i,
            correctionAnalysisDirect: /\b(?:what(?:'s| is| are)\s+(?:the\s+)?(?:error|mistake)s?|find\s+(?:the\s+)?(?:error|mistake)s?|identify\s+(?:the\s+)?(?:error|mistake)s?)\b[\s\S]{0,280}\b(?:correct|fix|rewrite)\b/i,
    ...
    ht: {
        ...
        tutor: {
            ...
            directionOptionsLettered: '`a) pratike vokabilè`, `b) pratike ak fraz`, `c) fè yon ti konvèsasyon`',
            translationExerciseBlocksInline: '',
            translationExerciseBlockCombinations: '',
            translationUnionMembers: '',
            quizTranslationItemUnion: '',
            ...
            conversationTitleRule: 'The title must be short, in Haitian Creole, human-friendly, and specific; avoid generic titles such as "Pratik anglè", "Konvèsasyon", or "Rezime konvèsasyon".',
        ...
        leakagePatterns: {
            ...
            orderSentences: /\b(?:ranje|mete)\b[\s\S]{0,180}\b(?:etap|fraz|enstriksyon|evènman)\b[\s\S]{0,60}\ban\s+l[oò]d\b/i,
            matching: /\b(?:marye|konekte|asosye)\b[\s\S]{0,180}\b(?:ak|chak|k[oò]r[eè]k|siyifikasyon|tradiksyon|p[eè])\b/i,
            multipleChoice: /\b(?:chwazi|make)\b[\s\S]{0,180}\b(?:bon\s+(?:opsyon|repons)|opsyon\s+k[oò]r[eè]k|repons\s+k[oò]r[eè]k)\b/i,
            ...
            correctionAnalysisPolite: /\b(?:[eè]ske\s+ou\s+ka(?:pab)?|ou\s+ka(?:pab)?)\s+(?:di\s+m|idantifye|montre|jwenn|eksplike)\b[\s\S]{0,280}\ber[eè][\s\S]{0,280}\bkorije\b/i,
            correctionAnalysisDirect: /\b(?:ki(?:l[eè]s)?\s+er[eè]|jwenn\s+er[eè]|idantifye\s+er[eè])[\s\S]{0,280}\bkorije\b/i,
...
export const supportedLocales = Object.keys(languages);
