...
- Update `system.md` and `message.md` to say:
  - lettered lists in `message` are allowed only for navigation choices
  - they must not have a hidden correct answer
  - any evaluable choice with correct answers must use `multiple_choice`
